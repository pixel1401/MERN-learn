...
import { Box, Divider, Modal, TextField, Typography, useTheme } from "@mui/material";
...
const UserWidget: FC<UserWidget> = ({ idAnotherUser, anotherUserInfo }) => {
    ...
    const { data: userInfo, error , refetch  } = useGetUserInfoQuery((idAnotherUser ?? userId ?? '').toString(), {
        ...
        <Box>
            <WidgetWrapper height={'fit-content'}>
                <FlexBetween>
                    ...
                    <ManageAccountsOutlined onClick={() => setIsUserModal(true)} />
                </FlexBetween>
            ...
            {userInfo != undefined && <EditUserWidget
                userInfo={userInfo}
                isUserModal={isUserModal}
                closeUserModal={() => setIsUserModal(false)}
                handleIsReload={() => handleIsReload()}
            />}
        </Box>
    )
}
